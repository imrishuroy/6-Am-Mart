...
// TODO: working on list thing
  Future<List<Item>> getStoreItemCategoryWise(
      {int storeID, int offset, int categoryIndex, String type}) async {
    try {
      print(
          'get store item category wise  storeID - $storeID | offset - $offset | categoryIndex - $categoryIndex | type - $type');
      //   List<Item> itemList = [];
      Response response = await storeRepo.getStoreItemList(
        storeID,
        offset,
        categoryIndex,
        // (_store != null && _store.categoryIds.length > 0 && categoryIndex != 0)
        //     ? _categoryList[categoryIndex].id
        //     : 0,
        type,
      );

      print('getStoreItemCategoryWise ${response.body}');

      if (response.statusCode == 200) {
        final itemModel = ItemModel.fromJson(response.body);

        print('Item modl ribsa -- ${itemModel.toString()}');

        return ItemModel.fromJson(response.body).items;
      }
      ...
      print('Error in getting store items ${error.toString()}');
      return [];
    }
  }
  ...
  Future<void> getStoreItemList(
    ...
    print(
        'Checking store item list StoreID -  $storeID | Offset $offset | type $type, | notify $notify');
    Response response = await storeRepo.getStoreItemList(
      storeID,
      offset,
